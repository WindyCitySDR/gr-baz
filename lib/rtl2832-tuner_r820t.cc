...
r820t_fci_result_type _r820t_i2c_read(RTL2832_NAMESPACE::tuner* pTuner, uint8_t *data, int len,
        const char* function = NULL, int line_number = -1, const char* line = NULL)
{
        int r = pTuner->i2c_read(R820T_I2C_ADDR, data, len);

        if (r <= 0)
        {
                DEBUG_TUNER_I2C(pTuner,r);
                return R820T_FCI_FAIL;
        }

        return R820T_FCI_SUCCESS;
}

r820t_fci_result_type _r820t_i2c_write(RTL2832_NAMESPACE::tuner* pTuner, uint8_t *data, int len,
        const char* function = NULL, int line_number = -1, const char* line = NULL)
{
        int r = pTuner->i2c_write(R820T_I2C_ADDR, data, len);
...
r820t_fci_result_type _r820t_set_i2c_repeater(RTL2832_NAMESPACE::tuner* pTuner, int on)
{
        int r = pTuner->set_i2c_repeater(on, NULL, -1, NULL);
...
#define r820t_i2c_read(t,r,b)          _r820t_i2c_read(t,r,b,CURRENT_FUNCTION,__LINE__,"R820T_Read("#t", "#r", "#b")")
#define r820t_i2c_write(t,r,w)         _r820t_i2c_write(t,r,w,CURRENT_FUNCTION,__LINE__,"R820T_Write("#t", "#r", "#w")")
#define r820t_set_i2c_repeater(t,m)    _r820t_set_i2c_repeater(t,m)

///////////////////////////////////////////////////////////////////////////////

namespace RTL2832_NAMESPACE { namespace TUNERS_NAMESPACE {

static int _mapGainsR820T[] = {        // RRK
        0, 9, 14, 27, 37, 77, 87, 125, 144, 157, 166, 229, 254, 280,
	297, 328, 338, 372, 386, 402, 421, 434, 439, 445, 480, 496
};
...
r820t::r820t(demod* p)
	: tuner_skeleton(p)
{
        for (int i = 0; i < sizeof(_mapGainsR820T)/sizeof(_mapGainsR820T[0]); i++)
                m_gain_values.push_back((double)_mapGainsR820T[i]);

        values_to_range(m_gain_values, m_gain_range);

        //m_frequency_range     // ?

	//RRK don't know what to do with this yet.
...
int r820t::initialise(tuner::PPARAMS params /*= NULL*/)
{
	...
	if (r820t_Initialize(this) != R820T_FUNCTION_SUCCESS)
		return FAILURE;
...
int r820t::set_frequency(double freq)
{
	...
	if (r820t_SetRfFreqHz(this, (unsigned long)freq) != R820T_FUNCTION_SUCCESS)
		return FAILURE;
...
int r820t::set_gain(double gain)
{
	...
	if (once) {
        	// RRK Put in manual mode
		r820t_set_i2c_repeater(this, 1);
		r = R828_RfGainMode(this, 1);
		r820t_set_i2c_repeater(this, 0);

		if(r != R820T_FUNCTION_SUCCESS)
			return FAILURE;
		once = 0;
	}

	r820t_set_i2c_repeater(this, 1);
	r = R828_SetRfGain(this, gain);
	r820t_set_i2c_repeater(this, 0);
...
{
	if (R828_Init(pTuner) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
	if (r820t_SetStandardMode(pTuner, DVB_T_6M) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
	return R820T_FUNCTION_SUCCESS;
}
...
//	R828Info.R828_Standard = (R828_Standard_Type)pExtra->StandardMode;
	...
	if(R828_SetFrequency(pTuner, R828Info, NORMAL_MODE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
{
	if(R828_SetStandard(pTuner, (R828_Standard_Type)StandardMode) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
int r820t_SetStandby(RTL2832_NAMESPACE::tuner* pTuner, int LoopThroughType)
{
	...
	if(R828_Standby(pTuner, (R828_LoopThrough_Type)LoopThroughType) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type
I2C_Write_Len(RTL2832_NAMESPACE::tuner* pTuner, R828_I2C_LEN_TYPE *I2C_Info)
{
...
//			goto error_status_set_tuner_registers;
		...
		if (r820t_i2c_write(pTuner, WritingBuffer, WritingByteNum + 1) != R820T_FCI_SUCCESS)
			return R820T_FUNCTION_ERROR;
	}
...
r820t_fci_result_type
I2C_Read_Len(RTL2832_NAMESPACE::tuner* pTuner, R828_I2C_LEN_TYPE *I2C_Info)
{
	uint8_t DeviceAddr;
...
//	if(pI2cBridge->ForwardI2cWritingCmd(pI2cBridge, DeviceAddr, &RegStartAddr, LEN_1_BYTE) != R820T_FUNCTION_SUCCESS)
//		goto error_status_set_tuner_register_reading_address;

	if (r820t_i2c_write(pTuner, &RegStartAddr, 1) != R820T_FCI_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
//	if(pI2cBridge->ForwardI2cReadingCmd(pI2cBridge, DeviceAddr, ReadingBytes, ByteNum) != R820T_FUNCTION_SUCCESS)
//		goto error_status_get_tuner_registers;

	if (r820t_i2c_read(pTuner, ReadingBytes, ByteNum) != R820T_FCI_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type
I2C_Write(RTL2832_NAMESPACE::tuner* pTuner, R828_I2C_TYPE *I2C_Info)
{
	...
	// Set tuner register bytes with writing buffer.
//	if(pI2cBridge->ForwardI2cWritingCmd(pI2cBridge, DeviceAddr, WritingBuffer, LEN_2_BYTE) != R820T_FUNCTION_SUCCESS)
//		goto error_status_set_tuner_registers;

	if (r820t_i2c_write(pTuner, WritingBuffer, 2) != R820T_FCI_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
//                   Internal Functions                     //
//----------------------------------------------------------//
r820t_fci_result_type R828_Xtal_Check(RTL2832_NAMESPACE::tuner* pTuner);
r820t_fci_result_type R828_Init(RTL2832_NAMESPACE::tuner* pTuner);
r820t_fci_result_type R828_InitReg(RTL2832_NAMESPACE::tuner* pTuner);
r820t_fci_result_type R828_IMR_Prepare(RTL2832_NAMESPACE::tuner* pTuner);
r820t_fci_result_type R828_IMR(RTL2832_NAMESPACE::tuner* pTuner, UINT8 IMR_MEM, int IM_Flag);
r820t_fci_result_type R828_PLL(RTL2832_NAMESPACE::tuner* pTuner, UINT32 LO_Freq, R828_Standard_Type R828_Standard);
r820t_fci_result_type R828_MUX(RTL2832_NAMESPACE::tuner* pTuner, UINT32 RF_KHz);
r820t_fci_result_type R828_IQ(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* IQ_Pont);
r820t_fci_result_type R828_IQ_Tree(RTL2832_NAMESPACE::tuner* pTuner, UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R828_SectType* CompareTree);
r820t_fci_result_type R828_CompreCor(R828_SectType* CorArry);
r820t_fci_result_type R828_CompreStep(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* StepArry, UINT8 Pace);
r820t_fci_result_type R828_Muti_Read(RTL2832_NAMESPACE::tuner* pTuner, UINT8 IMR_Reg, UINT16* IMR_Result_Data);
r820t_fci_result_type R828_Section(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* SectionArry);
r820t_fci_result_type R828_F_IMR(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* IQ_Pont);
r820t_fci_result_type R828_IMR_Cross(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* IQ_Pont, UINT8* X_Direct);
...
r820t_fci_result_type R828_Filt_Cal(RTL2832_NAMESPACE::tuner* pTuner, UINT32 Cal_Freq,BW_Type R828_BW);
...
r820t_fci_result_type R828_Xtal_Check(RTL2832_NAMESPACE::tuner* pTuner)
{
	...
	if(I2C_Write(pTuner, &R828_I2C) != R820T_FUNCTION_SUCCESS)
	    return R820T_FUNCTION_ERROR;
	...
	R828_I2C_Len.Len     = 3;
	if(I2C_Read_Len(pTuner, &R828_I2C_Len) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
//	pExtra = &(pTuner->Extra.R820t);
    ...
    //write initial reg
	//if(R828_InitReg(pTuner) != R820T_FUNCTION_SUCCESS)         
	//	return R820T_FUNCTION_ERROR;
...
//		  return R820T_FUNCTION_ERROR;
	  ...
	  {
          if(R828_Xtal_Check(pTuner) != R820T_FUNCTION_SUCCESS)        //1st
	          return R820T_FUNCTION_ERROR;

		  Xtal_cap_sel = Xtal_cap_sel_tmp;
		  
		  if(R828_Xtal_Check(pTuner) != R820T_FUNCTION_SUCCESS)        //2nd
	          return R820T_FUNCTION_ERROR;
		  ...
		  if(R828_Xtal_Check(pTuner) != R820T_FUNCTION_SUCCESS)        //3rd
	          return R820T_FUNCTION_ERROR;
...
#if 0
	  //start imr cal.
	  if(R828_InitReg(pTuner) != R820T_FUNCTION_SUCCESS)        //write initial reg before doing cal
	      return R820T_FUNCTION_ERROR;

	  if(R828_IMR_Prepare(pTuner) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;

	  if(R828_IMR(pTuner, 3, TRUE) != R820T_FUNCTION_SUCCESS)       //Full K node 3
		return R820T_FUNCTION_ERROR;

	  if(R828_IMR(pTuner, 1, FALSE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;

	  if(R828_IMR(pTuner, 0, FALSE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;

	  if(R828_IMR(pTuner, 2, FALSE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;

	  if(R828_IMR(pTuner, 4, FALSE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_InitReg(RTL2832_NAMESPACE::tuner* pTuner)
{
	...
	if(I2C_Write_Len(pTuner, &R828_I2C_Len) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_IMR(RTL2832_NAMESPACE::tuner* pTuner, UINT8 IMR_MEM, int IM_Flag)
{
	...
	//Must do before PLL() 
	if(R828_MUX(pTuner, RingFreq - 5300) != R820T_FUNCTION_SUCCESS)				//MUX input freq ~ RF_in Freq
		return R820T_FUNCTION_ERROR;

	if(R828_PLL(pTuner, (RingFreq - 5300) * 1000, STD_SIZE) != R820T_FUNCTION_SUCCESS)                //set pll freq = ring freq - 6M
	    return R820T_FUNCTION_ERROR;

	if(IM_Flag == TRUE)
	{
	if(R828_IQ(pTuner, &IMR_POINT) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
	}
	else
	{
		...
		if(R828_F_IMR(pTuner, &IMR_POINT) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
	}
...
//	R820T_EXTRA_MODULE *pExtra;
	...
	//FIXME hack
	R828_Arry[11] &= 0xEF;
	//RRK PLL_Ref = rtlsdr_get_tuner_clock(pTuner);
	PLL_Ref = R828_Xtal * 1000;
	...
	//fprintf(stderr, "VCO_Freq = %lu, Nint= %u, VCO_Fra= %lu, LO_Freq= %u, MixDiv= %u\n", VCO_Freq, Nint, VCO_Fra, LO_Freq, MixDiv);
...
//	UINT8 IQ_Count  = 0;
	...
	for(VGA_Count = 12;VGA_Count < 16;VGA_Count ++)
	{
		...
		if(R828_Muti_Read(pTuner, 0x01, &VGA_Read) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
	...
	//{
	    // Determine X or Y
	    if(R828_IMR_Cross(pTuner, &Compare_IQ[0], &X_Direction) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;

		//if(X_Direction==1)
		//{
		//    if(R828_IQ_Tree(Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //X
		//	  return R820T_FUNCTION_ERROR;
		//}
		//else
		//{
		//   if(R828_IQ_Tree(Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //Y
		//	return R820T_FUNCTION_ERROR;
		//}
		...
		//--- X direction ---//
	    //X: 3 points
		if(R828_IQ_Tree(Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //
			return R820T_FUNCTION_ERROR;

		//compare and find min of 3 points. determine I/Q direction
		if(R828_CompreCor(&Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;

		//increase step to find min value of this direction
		if(R828_CompreStep(&Compare_IQ[0], 0x08) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
		*/
		...
		if(X_Direction==1)
		{
		    ...
		    //increase step to find min value of this direction
		    if(R828_CompreStep(pTuner, &Compare_IQ[0], 0x08) != R820T_FUNCTION_SUCCESS)  //X
			  return R820T_FUNCTION_ERROR;
		}
		else
		{
		   ...
		   //increase step to find min value of this direction
		   if(R828_CompreStep(pTuner, &Compare_IQ[0], 0x09) != R820T_FUNCTION_SUCCESS)  //Y
			 return R820T_FUNCTION_ERROR;
		}
		/*
		//--- Y direction ---//
		//Y: 3 points
		if(R828_IQ_Tree(Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //
			return R820T_FUNCTION_ERROR;
		...
		//increase step to find min value of this direction
		if(R828_CompreStep(&Compare_IQ[0], 0x09) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
        */
		...
		if(X_Direction==1)
		{	    
           if(R828_IQ_Tree(pTuner, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //Y
		     return R820T_FUNCTION_ERROR;
		...
		{
		   if(R828_IQ_Tree(pTuner, Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS) //X
		     return R820T_FUNCTION_ERROR;
	...
	{
		if(R828_Section(&Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
	...
	*/
    ...
    //Section-9 check
    //if(R828_F_IMR(&Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)
	if(R828_Section(pTuner, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_IQ_Tree(RTL2832_NAMESPACE::tuner* pTuner, UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R828_SectType* CompareTree)
{
	...
	for(TreeCount = 0;TreeCount < TreeTimes;TreeCount ++)
	{
		...
		if(R828_Muti_Read(pTuner, 0x01, &CompareTree[TreeCount].Value) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_CompreStep(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* StepArry, UINT8 Pace)
{
	...
	while(((StepTemp.Gain_X & 0x1F) < IMR_TRIAL) && ((StepTemp.Phase_Y & 0x1F) < IMR_TRIAL))  //5->10
	{
		...
		if(R828_Muti_Read(pTuner, 0x01, &StepTemp.Value) != R820T_FUNCTION_SUCCESS)
			return R820T_FUNCTION_ERROR;
...
// output: TRUE or FALSE
//-----------------------------------------------------------------------------------/
r820t_fci_result_type R828_Muti_Read(RTL2832_NAMESPACE::tuner* pTuner, UINT8 IMR_Reg, UINT16* IMR_Result_Data)  //jason modified
{
	UINT8 ReadCount;
	UINT16 ReadAmount;
	UINT8 ReadMax;
...
r820t_fci_result_type R828_Section(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* IQ_Pont)
{
	R828_SectType Compare_IQ[3];
	...
	if(R828_IQ_Tree(pTuner, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)  // y-direction
		return R820T_FUNCTION_ERROR;
	...
	if(R828_IQ_Tree(pTuner, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
	...
	if(R828_CompreCor(&Compare_Bet[0]) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_IMR_Cross(RTL2832_NAMESPACE::tuner* pTuner, R828_SectType* IQ_Pont, UINT8* X_Direct)
{
	...
	for(CrossCount=0; CrossCount<5; CrossCount++)
	{
        ...
        if(R828_Muti_Read(pTuner, 0x01, &Compare_Cross[CrossCount].Value) != R820T_FUNCTION_SUCCESS)
		  return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_SetStandard(RTL2832_NAMESPACE::tuner* pTuner, R828_Standard_Type RT_Standard)
{
    ...
    if(R828_Fil_Cal_flag[RT_Standard] == FALSE)
	{
		...
		if(R828_Filt_Cal(pTuner, Sys_Info1.FILT_CAL_LO,Sys_Info1.BW) != R820T_FUNCTION_SUCCESS)
		    return R820T_FUNCTION_ERROR;
...
*/
	...
	if(R828_PLL(pTuner, Cal_Freq * 1000, STD_SIZE) != R820T_FUNCTION_SUCCESS)
		return R820T_FUNCTION_ERROR;
...
r820t_fci_result_type R828_SetFrequency(RTL2832_NAMESPACE::tuner* pTuner, R828_Set_Info R828_INFO, R828_SetFreq_Type R828_SetFreqMode)
{
	UINT32	LO_Hz;
     ...
     if(R828_MUX(pTuner, LO_Hz/1000) != R820T_FUNCTION_SUCCESS)
        return R820T_FUNCTION_ERROR;

     //Set PLL
     if(R828_PLL(pTuner, LO_Hz, R828_INFO.R828_Standard) != R820T_FUNCTION_SUCCESS)
        return R820T_FUNCTION_ERROR;
